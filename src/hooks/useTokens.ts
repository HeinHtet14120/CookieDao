...
const QUICKNODE_RPC = {
    solana: "https://tiniest-broken-lake.solana-mainnet.quiknode.pro/c5462950ebb302a25357758b0160085153b91d73/",
  ...
  useEffect(() => {
    if (!walletAddress) return;
        ...
        const fetchAllTokens = async () => {
            try {
                setLoading(true);
                console.log("🔹 Fetching contract addresses from CookieFun API...", walletAddress);

                // ✅ Fetch **ALL** Contract Addresses from CookieFun API
                const allTokens = await fetchAllContractAddresses();
                if (allTokens.length === 0) throw new Error("No tokens found in CookieFun API.");

                console.log(`✅ Successfully fetched ${allTokens.length} tokens from CookieFun API`);

                // ✅ Fetch Wallet Balances using QuickNode API
                console.log("🔹 Fetching balances from QuickNode...");
                const walletBalances = await fetchWalletBalances(walletAddress, allTokens);
...
const fetchAllContractAddresses = async () => {
    try {
        let allContractAddresses: string[] = []; // Array to store all contract addresses
        let page = 1;
                ...
                if (response.success && response.ok.data) {
                    response.ok.data.forEach((token) => {
                        token.contracts.forEach((contract) => {
                            ...
                            console.log(`🔹 Page ${page} Contract Address: ${contract.contractAddress}`);
                        });
                    });
        ...
        console.log(`✅ Successfully fetched ${allContractAddresses.length} contract addresses.`);
        ...
        for (const token of allTokens) {
            ...
            try {
                if (tokenChain === "solana") {
                    ...
                    const provider = new ethers.JsonRpcProvider(QUICKNODE_RPC[tokenChain]);
                ...
                console.error(`❌ Error fetching balance for ${tokenContract} on chain ${tokenChain}:`, fetchError);
            }
        }

        console.log(`✅ Successfully fetched ${tokenBalances.length} wallet balances`);
