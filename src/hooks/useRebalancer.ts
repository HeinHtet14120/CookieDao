...
export const useRebalancer = () => {
    ...
    const [allocations, setAllocations] = useState<{ token: string; allocation: number }[]>([]);
    ...
    const setAllocation = async (token: string, allocation: number) => {
        ...
        setLoading(true);
        try {
            const tx = await contract.setTokenAllocation(token, parseUnits(allocation.toString(), 0));
            await tx.wait();
    ...
    const suggestSwap = async (fromToken: string, toToken: string, amount: number) => {
        if (!contract) return;
        setLoading(true);
        try {
            const tx = await contract.suggestSwap(fromToken, toToken, parseUnits(amount.toString(), 18));
            await tx.wait();
        } catch (err) {
    ...
    return { contract, allocations, setAllocation, suggestSwap, fetchAllocations, loading, error, walletAddress };
};
